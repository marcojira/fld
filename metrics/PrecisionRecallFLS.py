...
class PrecisionRecallFLS(Metric):
    ...
    def get_nn_dists(self, feat, other_feat):
        dists = torch.cdist(feat, other_feat)
        dists = dists.topk(self.num_neighbors, dim=1, largest=False).values
        return dists[:, -1]

    def pct_in_manifold(self, dist_feat, manifold_feat, eval_feat):
        nn_dists = self.get_nn_dists(manifold_feat, dist_feat)
        pairwise_dists = torch.cdist(eval_feat, manifold_feat)
        ...
        return num_in_manifold / eval_feat.shape[0]
    ...
    def compute_metric(
        ...
        if self.mode == "Precision":
            return self.pct_in_manifold(gen_feat, train_feat, test_feat).cpu().item()
        elif self.mode == "Recall":
            return self.pct_in_manifold(train_feat, gen_feat, test_feat).cpu().item()
        else:
